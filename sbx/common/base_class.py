...
from sbx.common.monitor import Monitor
from sbx.common.policies import BaseJaxPolicy
from sbx.common.gymanax_wrapper import GymnaxToVectorGymWrapper as GymanaxVecEnv
import gymnax as gym
SelfBaseAlgorithm = TypeVar("SelfBaseAlgorithm", bound="BaseAlgorithm")

from sbx.common.type_aliases import Schedule, MaybeCallback, GymEnv, JnpDict, Env
def maybe_make_env(env: Union[Env, str], verbose: int,env_kwargs: Optional[Dict[str, Any]],seed:Optional[int] =None ) -> GymEnv:
    """If env is a string, make the environment; otherwise, return env.

    :param env: The gymnax environment to learn from.
    ...
    if isinstance(env, str):
        ...
        try:
            ...
            env, env_params  = gym.make(env_id, **env_kwargs)
        except TypeError:
            env,env_params  = gym.make(env_id,**env_kwargs)
    env = GymanaxVecEnv(env, num_envs=num_envs,seed=seed,params=env_params)
    assert isinstance(env, GymanaxVecEnv), "Error: the environment passed is not a Gym environment."
    return env

class BaseAlgorithmJax(ABC):
    """
    ...
    policy_aliases: Dict[str, Type[BaseJaxPolicy]] = {}
    policy: BaseJaxPolicy
    ...
    def __init__(
        self,
        policy: Union[str, Type[BaseJaxPolicy]],
        ...
        self._last_obs = None  # type: Optional[Union[jnp.ndarray, Dict[str, jnp.ndarray]]]
        self._last_episode_starts = None  # type: Optional[jnp.ndarray]
        # When using VecNormalize:
        self._last_original_obs = None  # type: Optional[Union[jnp.ndarray, Dict[str, jnp.ndarray]]]
        self._episode_num = 0
        # Used for gSDE only
        ...
        if env is not None:
            ...
            env = maybe_make_env(env,self.verbose,env_kwargs, seed)
      #      env = self._wrap_env(env, self.verbose, monitor_wrapper,num_envs=num_envs)
            ...
            if isinstance(self.action_space, spaces.Box):
                assert jnp.all(
                    jnp.isfinite(jnp.array([self.action_space.low, self.action_space.high]))
                ), "Continuous action space must have a finite lower and upper bound"

    @staticmethod
    def _wrap_env(env: GymEnv, verbose: int = 0, monitor_wrapper: bool = True,num_envs=1) -> VecEnv:
        """ "
    ...
    def _get_policy_from_name(self, policy_name: str) -> Type[BaseJaxPolicy]:
        """
    ...
    def _update_info_buffer(self, infos: Dict[str, Any], dones: Optional[jnp.ndarray] = None) -> None:
        """
        ...
        if dones is None:
            dones = jnp.array([False] * len(infos))
        maybe_ep_info = infos.get("episode")
        maybe_is_success = infos.get("is_success")
        if maybe_ep_info is not None:
            self.ep_info_buffer.extend(maybe_ep_info)
        if maybe_is_success is not None:
            self.ep_success_buffer.append(jnp.logical_and(dones, maybe_is_success))
    ...
    def set_parameters(
        self,
        load_path_or_dict: Union[str, JnpDict],
