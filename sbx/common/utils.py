...
from sbx.common.type_aliases import GymEnv, Schedule, JnpDict, TrainFreq, TrainFrequencyUnit
...
def explained_variance(y_pred: jnp.ndarray, y_true: jnp.ndarray) -> jnp.ndarray:
    """
...
def is_vectorized_box_observation(observation: jnp.ndarray, observation_space: spaces.Box) -> bool:
    """
...
def is_vectorized_discrete_observation(observation: Union[int, jnp.ndarray], observation_space: spaces.Discrete) -> bool:
    """
...
def is_vectorized_multidiscrete_observation(observation: jnp.ndarray, observation_space: spaces.MultiDiscrete) -> bool:
...
def is_vectorized_multibinary_observation(observation: jnp.ndarray, observation_space: spaces.MultiBinary) -> bool:
...
def is_vectorized_dict_observation(observation: jnp.ndarray, observation_space: spaces.Dict) -> bool:
    """
...
def is_vectorized_observation(observation: Union[int, jnp.ndarray], observation_space: spaces.Space) -> bool:
    """
...
def safe_mean(arr: Union[jnp.ndarray, list, deque]) -> jnp.ndarray:
    """
    ...
    :return:
    """
    return jnp.nan if len(arr) == 0 else jnp.mean(arr)
