...
class NormalActionNoise(ActionNoise):
    ...
    def __init__(self, mean: jnp.ndarray, sigma: jnp.ndarray, seed=0,dtype: DTypeLike = jnp.float32) -> None:
        self._mu = mean
        self._sigma = sigma
        self._dtype = dtype
    ...
    def __call__(self) -> jnp.ndarray:
        return jax.random.normal(self.noise_key,self._mu.shape).astype(self._dtype) * self._sigma + self._mu
...
class OrnsteinUhlenbeckActionNoise(ActionNoise):
    ...
    def __init__(
        ...
        self.noise_prev = jnp.zeros_like(self._mu)
        self.reset()
        super().__init__()
    ...
    def __call__(self) -> jnp.ndarray:
        noise = (
            ...
            + self._sigma * jnp.sqrt(self._dt) * jnp.random.normal(self.noise_key,shape=self._mu.shape)
        )
    ...
    def reset(self) -> None:
        ...
        """
        self.noise_prev = self.initial_noise if self.initial_noise is not None else jnp.zeros_like(self._mu)
...
class VectorizedActionNoise(ActionNoise):
    ...
    def __call__(self) -> jnp.ndarray:
        ...
        """
        noise = jnp.stack([noise() for noise in self.noises])
        return noise
