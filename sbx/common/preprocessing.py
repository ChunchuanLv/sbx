...
def is_image_space(
    ...
    if isinstance(observation_space, spaces.Box) and len(observation_space.shape) == 3:
        # Check the type
        if check_dtype and observation_space.dtype != jnp.uint8:
            return False

        # Check the value range
        incorrect_bounds = jnp.any(observation_space.low != 0) or jnp.any(observation_space.high != 255)
...
def maybe_transpose(observation: jnp.ndarray, observation_space: spaces.Space) -> jnp.ndarray:
    """
...
def get_flattened_obs_dim(observation_space: spaces.Space) -> int:
    ...
    if isinstance(observation_space, spaces.Box):
        ...
        return sum([get_action_dim(subspace) for subspace in observation_space.spaces])
    elif isinstance(observation_space, spaces.Dict):
        return sum([get_action_dim(subspace) for subspace in observation_space.spaces.values()])
    if isinstance(observation_space, spaces.Discrete):
        return sum(observation_space.nvec)
    else:
        raise NotImplementedError(f"{observation_space} action space is not supported")
...
def get_action_dim(action_space: spaces.Space) -> int:
    ...
    if isinstance(action_space, spaces.Box):
        ...
        return sum([get_action_dim(subspace) for subspace in action_space.spaces])
    elif isinstance(action_space, spaces.Dict):
        return sum([get_action_dim(subspace) for subspace in action_space.spaces.values()])
