...
from sbx.common.type_aliases import Schedule
...
class HSACPolicy(BaseJaxPolicy):
    ...
    def build(self, key: jax.random.KeyArray, lr_schedule: Schedule, qf_learning_rate: float) -> jax.random.KeyArray:
        ...
        if isinstance(self.observation_space, spaces.Dict):
            obs = jnp.array([spaces.flatten(self.observation_space, self.observation_space.sample(key))])
        else:
            obs = jnp.array([self.observation_space.sample(key)])
        action = jnp.array([self.action_space.sample(key)])
    ...
    def _predict(self, observation: jnp.ndarray, deterministic: bool = False) -> jnp.ndarray:  # type: ignore[override]
        if deterministic:
