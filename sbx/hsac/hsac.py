...
class HSAC(OffPolicyAlgorithmJax):
    ...
    def _setup_model(self) -> None:
        ...
        if not hasattr(self, "policy") or self.policy is None or True:
       ...
       #     assert isinstance(self.qf_learning_rate, Union[float, Schedule])
    ...
    def train(self, batch_size, gradient_steps):
        ...
        self.key, data_key = jax.random.split(self.key, 2)
        data = self.replay_buffer.sample(data_key,batch_size * gradient_steps, env=self._vec_normalize_env)
        ...
        if isinstance(data.observations, dict):
            ...
            obs = jnp.concatenate([data.observations[key] for key in keys], axis=1)
            next_obs = jnp.concatenate([data.next_observations[key] for key in keys], axis=1)
        else:
  ...
  #      print ("dones",data.dones.shape)
   #     print ("rewards",data.rewards.shape)
    ...
    @staticmethod
    @jax.jit
    def update_temperature(target_entropy: jnp.ndarray, ent_coef_state: TrainState, entropy: float):
