...
def evaluate_hyper_parameters(log_train_freq=0,log_gradient_steps=None,gamma=0.9):
    ...
    model = SAC("MlpPolicy", env_name, gamma=gamma,seed=random.seed(),
...
#unsupervised = True
#evaluate_hyper_parameters(log_train_freq=4,log_gradient_steps=2)
pbounds={
  #  "log_learning_rate": (-8, -4),
    "log_train_freq": (0.0, 5),
  #  "log_gradient_steps": (0.0, 5),
   # "gamma": (0.9, 0.99),
...
logger = JSONLogger(path="./sac_"+env_name+"_"+"_".join(pbounds.keys())+"_logs")
